...
void KmeansImage::AssignCluster(RgbImage* image, Clusters* clusters, int* counter) {
	std::stringstream ss;
...
void KmeansImage::segmentImage(RgbImage* image, Clusters* clusters, int n) {
	...
	for (int i = 1; i < n; ++i) {
		...
		v0_ = v1.init(call_num2, image->h*image->w*rate);
		auto tpb1 = std::bind(&KmeansSchemeFluidX::AssignCluster, this, image, clusters, std::placeholders::_1);
		auto tp1 = tf->newTask<decltype(tpb1), int*>("AssignCluster" + std::to_string(i), tpb1, call_num1->p);
		Guard* g1 = gs->newGuard("GuardAssignCluster" + std::to_string(i), { v0_ }, tp1, {});
