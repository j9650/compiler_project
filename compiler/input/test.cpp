...
void KmeansImage::AssignCluster(RgbImage_new* image, Clusters_new* clusters) {
	std::stringstream ss;
	...
	for (int y = 0; y < image->h; y++) {
		for (int x = 0; x < image->w; x++) {
			...
			#pragma CallCount {assignCluster(&image->pixels[yy][xx], clusters);}
		}
	}
}
void KmeansImage::AssignCluster_stable(RgbImage_new* image, Clusters_new* clusters) {
	image->iteration++;
	std::stringstream ss;
	srand((unsigned)std::chrono::system_clock::now().time_since_epoch().count());
	for (int y = 0; y < image->h; y++) {
		for (int x = 0; x < image->w; x++) {
			...
			//#pragma CallCount {<<<g0,{ValveST(p->cluster,STABLE_TH)},{}>>>assignCluster(p, clusters);}

			//if((p->stable <= STABLE_TH)||((randg1/image->size) % 7 < 3)) {
			//	auto j=p->cluster;
			//	assignCluster(p, clusters); //printf("p->stable:%d\n", p->stable);}
			//	if(p->cluster == j)
			//		p->stable++;
			//	else
			//		p->stable=0;
...
void KmeansImage::segmentImage(RgbImage_new* image, Clusters_new* clusters, int n) {
	...
	#pragma CallCount {__call_count__ call_num1;}
	...
	<<<g1,{},{},call_num1>>>AssignCluster(image,clusters);
	...
	<<<g2,{v1(call_num1, image->h*image->w*rate), v1(call_num1, image->h*image->w*rate) },{}>>>Recenter(image,clusters);

	//auto call_num2 = call_num1;
	#pragma CallCount {__call_count__ call_num2 = call_num1;}
	for (int i = 1; i < n; ++i) {
		...
		<<<g1,{v1(call_num2, image->h*image->w*rate)},{},call_num1>>>AssignCluster(image,clusters);
		...
		<<<g2,{v1(call_num1, image->h*image->w*rate)},{}>>>Recenter(image,clusters);

		//call_num2 = call_num1;
		#pragma CallCount {call_num2 = call_num1;}
	}

	//int* counter = new int(0);
	////#pragma CallCount {__call_count__ counter;}
	//auto tpb__g3 = std::bind(&KmeansImage::AssignCluster_stable, this, image, clusters, std::placeholders::_1);
	//auto tp__g3 = tf->newTask<decltype(tpb__g3), int*>("AssignCluster" + std::to_string(i), tpb__g3, counter);
	//Guard* g3 = gs->newGuard("GuardAssignCluster" + std::to_string(i), {  }, tp__g3, {});
	////<<<g3,{},{},counter>>>AssignCluster_stable(image, clusters)
